...
class Linear(object):
    ...
    def forward(x, w, b):
        ...
        # The view function is used to reshape the input tensor x into rows while maintaining the original number of columns.
        # The -1 argument in the view function is used to automatically infer the size of the second dimension.
        x_reshaped = x.view(x.size(0), -1)

        # Forward pass
        # The torch.matmul function is used for matrix multiplication between the reshaped input x_reshaped and the weight matrix w.
        # The bias term b is added to the result using torch.unsqueeze and torch.expand to ensure compatibility with the input.
        out = torch.matmul(x_reshaped, w) + b.unsqueeze(0).expand(x.size(0), -1)
        ######################################################################
        #                        END OF YOUR CODE                            #
        ######################################################################
        cache = (x, w, b)
    ...
    def backward(dout, cache):
        ...
        # Use the transpose of weight matrix w to backpropagate gradients through the input x.
        # Reshape dx back to the original shape of the input.
        dx = torch.mm(dout, w.t()).view(*x.shape)

        # Gradient of loss with respect to w
        # Use the transpose of the reshaped input x_reshaped to compute the gradients with respect to weight matrix w.
        dw = torch.mm(x_reshaped.t(), dout)

        # Gradient of loss with respect to b
        # Calculate the gradient of the loss with respect to bias term b by summing along axis 0.
...
class ReLU(object):
    ...
    def backward(dout, cache):
        ...
        # The gradient of the ReLU function is 1 for positive values in x and 0 for negative values.
        # This is achieved by using a binary mask where elements greater than 0 are set to 1, and others to 0.
        dx = dout * (x > 0).float()
        #####################################################
        #                  END OF YOUR CODE                 #
        #####################################################
...
class Linear_ReLU(object):
    ...
    def forward(x, w, b):
        ...
        ######################################################################
        # Replace "pass" statement with your code
        # Forward pass through a linear layer followed by ReLU activation
        # Reshape the input x into rows and perform matrix multiplication with weight matrix w.
        # Add the bias term b and apply the ReLU activation function element-wise.
        out = torch.relu(torch.matmul(x.view(x.shape[0], -1), w) + b)

        # Cache intermediate values for backward pass
        # The tuple (x, w, b, linear_output) is stored to be used in the backward pass for computing gradients.
        cache = (x, w, b, torch.matmul(x.view(x.shape[0], -1), w) + b)
    ...
    def backward(dout, cache):
        ...
        # Compute the gradient of ReLU function during backward pass
        # The gradient is 1 for positive values in fc_out and 0 for negative values.
        # This is achieved by using a binary mask where elements greater than 0 are set to 1, and others to 0.
        drelu = dout * (fc_out > 0).float()

        # Backward pass to compute gradients for x, w, and b
        # Gradient of loss with respect to x
        dx = torch.matmul(drelu, w.t()).view(x.shape)

        # Gradient of loss with respect to w
        dw = torch.matmul(x.view(x.shape[0], -1).t(), drelu)

        # Gradient of loss with respect to b (sum along axis 0)
...
def softmax_loss(x, y):
    ...
    ######################################################################
    # Replace "pass" statement with your code
    # Number of samples in the input x
    N = x.shape[0]

    # Calculate softmax probabilities and handle numerical stability
    # Shift values by subtracting the maximum value along each row to prevent overflow in exponentiation.
    probs = torch.exp(x - torch.max(x, dim=1, keepdim=True)[0]) / torch.sum(torch.exp(x - torch.max(x, dim=1, keepdim=True)[0]), dim=1, keepdim=True)

    # Compute the cross-entropy loss
    # The negative log probability of the correct class is averaged over all samples.
    loss = torch.mean(-torch.log(probs[range(N), y]))

    # Compute the gradient of the loss with respect to x
    # The gradient is obtained by subtracting 1 from the predicted probability of the correct class for each sample.
    dx = probs.clone()
    dx[range(N), y] -= 1
    dx /= N
    ######################################################################
...
class TwoLayerNet(object):
    ...
    def __init__(self, input_dim=3*32*32, hidden_dim=100, num_classes=10,
        ...
        # The weights 'W1' connect the input layer to the hidden layer, initialized with random values.
        # The biases 'b1' are initialized as zeros for the hidden layer.
        # The weights 'W2' connect the hidden layer to the output layer, initialized with random values.
        # The biases 'b2' are initialized as zeros for the output layer.
    ...
    def loss(self, X, y=None):
        ...
        # Apply the first layer transformation: linear transformation followed by ReLU activation.
        h, cache = Linear_ReLU.forward(X, self.params["W1"], self.params["b1"])
        # Apply the second layer transformation: linear transformation to obtain the scores
        # Force scores to be of type float64 for numerical stability or specific requirements.
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        # Define the dimensions of the neural network layers, including input, hidden, and output layers
        dims = [input_dim] + hidden_dims + [num_classes]

        # Loop through each layer to initialize weights and biases
        for i in range(self.num_layers):
            # Initialize weights using a normal distribution with specified scale
    ...
    def loss(self, X, y=None):
        ...
        loss, dout = softmax_loss(scores, y)
        for i in range(self.num_layers):
            loss =  loss + self.reg * torch.sum(self.params[f'W{i+1}'] * self.params[f'W{i+1}'])
    
        for i in reversed(range(self.num_layers)):
            if i == self.num_layers - 1:
                cache = caches[i]
                dout, dw, grads[f'b{i+1}'] = Linear.backward(dout, cache)
                grads[f'W{i+1}'] = dw + 2 * self.reg * self.params[f'W{i+1}']
            else:
                cache = caches[i]
                dout, dw, grads[f'b{i+1}'] = Linear_ReLU.backward(dout, cache)
                grads[f'W{i+1}'] = dw + 2 * self.reg * self.params[f'W{i+1}']
